...
title: About
heading: About Aura
standfirst: A collection of independent, well-tested, semantically versioned library packages that can be used in any codebase.
---

The Aura project centers around a collection of high-quality, well-tested, [semantically versioned](http://semver.org), [standards-compliant]({{ site.baseurl }}/contributing#standards), independent library packages that can be used in any codebase.
...
Aura 1.x began as a rewrite of [Solar](http://solarphp.com), reimagined as a library collection with dependency injection instead of a monolithic framework with service location. (The name change from Solar to Aura was to reduce confusion with the Apache Solr project.) The libraries, once finished, were combined into the [1.x framework]({{ site.baseurl }}/framework/1.x/en/).

### 2.x

Aura 2.x continues the decoupling of 1.x components into even more independent packages. It also splits apart the framework into kernel and project packages, providing the basis for the [2.x framework]({{ site.baseurl }}/framework/2.x/en/). Various `*_Kernel` and `*_Project` packages compose the [2.x framework]({{ site.baseurl }}/framework/2.x/en); they depend on the foundational Aura libraries as well as external packages.
